...
class CsvProcessingService {
  ...
  async processFile(file, username) {
    ...
    try {
      ...
      console.log(`📋 Matched file to table: ${tableName}`);
  ...
  async importCsvToTable(filePath, tableName, columns, username) {
    ...
    try {
        ...
        transformStream.on('data', async (item) => {
          if (item.type === 'batch') {
            ...
            try {
              ...
              console.error(`❌ Error inserting batch at row ${processedCount}:`, error);
              errors.push({
                row: processedCount,
                error: error.message,
                batch: true,
      ...
      console.log(`✅ Imported ${insertedCount} rows to ${tableName} from CSV`);
    } catch (error) {
  ...
  async processBatch(files, username) {
    console.log(`🔄 Starting batch CSV import at 2025-04-07 02:17:31 by wilmer2000`);
